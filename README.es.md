...
![Ciclo del Motor Evolutivo, animado](docs/motor-evolutivo-loop.gif)
...
| **R7 VERIFICAR-PRE** | Antes de proponer "reparar X", verificar que X esté roto de verdad — como campo: toda jugada que asume un estado lleva `Premisa: … · visto: <comando> → <resultado>`; sin eso la jugada solo puede ser "verificar X". **R7-b:** antes de dictaminar la propuesta *de otro agente*, leer el código real que toca — y exigir la capacidad nueva antes que la prolijidad. **R7-c:** antes de escribir un handoff que instruye mecanismos concretos en una máquina ajena (git pull, script, servicio, deploy dir), verificar por efecto read-only que existen allá | Propuestas de arreglos fantasma sobre memoria desactualizada · veredictos escritos sobre un texto en vez de sobre el código |
...
- **32 mutaciones aprobadas** del prompt maestro (v1.0 → v3.13) en ~15 semanas, cada una fundada en ejecuciones reales — [historial completo fechado, sanitizado →](docs/CHANGELOG-HISTORY.md) (en inglés)
...
- **v3.12 - el autoconocimiento que fallo al primer intento se vuelve guard, no solo memoria (R9-c):** un contenido con backslashes, `\n`, backticks o `$` (una ruta estilo Windows, una regex, un script con codigo escapado) se escribe con la escritura directa del editor, o desde un archivo temporal que el script LEE - nunca inline por una shell intermedia (`printf`, un heredoc sin comillas, `python -c`, `node -e` con el texto adentro, `sed` con la ruta metida en el patron). Cada capa de shell consume una capa de escapes y ejecuta cualquier backtick de paso; el destino queda mutilado y el verde del comando no lo dice. El hallazgo ya estaba escrito como conocimiento (R9, v2.5) y aun asi disparo cinco veces en 48 horas: dos scripts inline que salieron mutilados, un wrapper de tarea programada que quedo sin sus separadores de ruta y fallo mudo, una ruta commiteada con los separadores comidos, una edicion de regex que dejo de matchear en silencio. Extiende el alcance de R9 (precedente v2.9/v3.0), no es regla nueva - pero esta vez la memoria sola no alcanzo, asi que tambien trae **su propio guard de pre-ejecucion**: un chequeo que inspecciona el comando de shell exacto antes de que corra y bloquea el patron. Prueba de rojo: saboteado contra 14 comandos reales sacados de la sesion que lo produjo (5 que deben bloquear, 9 legitimos que no) - caza un falso positivo en su propio primer borrador (un heredoc citado pasado a un interprete no lo expande la shell y se estaba bloqueando igual) y un punto ciego (una escritura multilinea escondia sus propios backslashes mas alla de donde el chequeo leia). Corregidos los dos, despues lo vio bloquear en vivo exactamente el tipo de comando que existe para frenar.
- **v3.11 - revision adversarial antes del recibo (R11-b):** un workflow reescrito (9→6 nodos) ya estaba medido en produccion y cerrado con recibo. Una sola pasada de otro modelo sobre el JSON pegado encontro 3 bugs reales - uno de semanas: caracteres de reemplazo (U+FFFD) adentro de las claves de un mapeo de columnas, asi que 3 columnas se escribian vacias desde siempre. El autor *habia mirado esas claves y las descarto* por analogia con un mojibake inofensivo visto antes. La misma semana, dos entregas de otro agente llegaron con self-test en verde y un agujero de seguridad cada una (bypass de auth por presencia de header; inyeccion en una consulta) que solo una lectura completa cazo. R11 decia "varia una dimension que no nombraste"; el autor no puede nombrar con confianza la dimension a la que es ciego. Entonces, para dos casos nada mas - un workflow reescrito, o codigo que toca una frontera de confianza - la dimension no nombrada la pone otra cabeza, antes del recibo, y el recibo cita su hallazgo mas grave. Acotada a dos casos para no gastar la cuota del revisor en fixes de un campo.
- **v3.10 - el rojo se corre donde vive el guard (R6-b ii):** un canario de deploy se habia "visto en rojo" tres veces - las tres corriendo el bloque suelto en una shell. Adentro del script real, bajo `set -euo pipefail`, un `grep` sobre un archivo de entorno inexistente devolvia 2 y mataba el deploy mudo en esa linea; esa misma manana, el probe del lado del contenedor decia "no se pudo probar (sin curl)" y dejaba pasar todo. Los dos rojos existian en el papel; ninguno se habia ejecutado nunca en el lugar donde el guard iba a vivir. Ahora la prueba de rojo solo cuenta si corrio en el mismo script, flags, contenedor y runner - y una rama "skip" es un rojo, no un pase.
- **v3.9 - el ejecutor se cablea en el cierre (R8):** la regla de routing decia que cada jugada nombra su ejecutor mas barato capaz - y funcionaba, en el papel. Hasta que una sesion recorrio el backlog buscando combustible para el loop autonomo de un agente delegado (uno que solo consume tickets etiquetados para el) y encontro **cero** tickets dirigidos a el, con varias jugadas "ejecutor: ese agente" en cierres previos. Una jugada en prosa nunca alimento el loop. Misma clase de drift que v3.7/v3.8: la regla gobernaba la *etiqueta*, no la *entrega*. Ahora una jugada delegada a un agente que vive de tickets se convierte en ticket en el mismo cierre, y una delegada a un modelo barato que vive de briefs lleva al lado el alias del modelo y el brief. El ejecutor mas barato del roster (un modelo abierto en la nube, "el albanil, no tiene que pensar tanto", en palabras del humano) entra explicitamente a la lista de routing, con brief cerrado como insumo.
- **v3.8 - las acciones fuera del teclado no son jugadas (R12-d):** dos tramos seguidos, el menu propuso cosas que solo el humano puede hacer lejos del teclado - pagar una suscripcion, llamar a alguien, entrar a un dashboard externo. No las hizo en medio de la sesion (obvio), el motor las conto como ignoradas, y la Efectividad dio 0.33 y 0.56 cuando las jugadas que el motor si podia ejecutar se tomaron casi todas. La regla "fuera del teclado no es jugada" existia desde la metrica v1.6 - en el paso de puntuar de una skill auxiliar, no en el prompt maestro que arma el menu. Mismo hallazgo que v3.7, un escalon mas abajo: la regla vivia donde se corrige el puntaje, no donde se toma la decision. Ahora van a un bloque acumulativo `⛔ Bloqueantes - en tu cancha`, cada una con receta clicable (palabras del humano: "muchas no se hacerlas - son tokens o muchos clicks"; "las junto todas y las hago de una sola vez"; "lo que no quiero es que se detengan ustedes"), y la jugada del motor es lo que pueda dejar listo mientras tanto.
...
- **Instalalo como plugin (dos comandos):**
  ```
  /plugin marketplace add Guille512/motor-evolutivo
  /plugin install motor-evolutivo@motor-evolutivo
  ```
  El ciclo completo (pasos 3-5) se opera diciendo "motor", sin copiar/pegar nada a mano. Los marketplaces de terceros no se actualizan solos por defecto: corré `/plugin marketplace update` para recibir versiones nuevas. ¿Preferís archivos? Copiá [`skills/motor-evolutivo/`](skills/motor-evolutivo/SKILL.md) a `~/.claude/skills/`.
...
├── skills/motor-evolutivo/SKILL.md      ← operador del ciclo para Claude Code
├── .claude-plugin/                       ← manifiesto de plugin + marketplace
