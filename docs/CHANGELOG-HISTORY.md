...
>
> **32 approved prompt mutations between 2026-06-10 and 2026-09-22.** Two extra entries
> (v1.1, v1.2.1) are infrastructure/process changes that did not touch the master prompt —
> listed for completeness, not counted.

---

- **v3.13 — approved 2026-09-22:** **the premise becomes a field of the play (R7).** Every play that assumes a state ("X is broken/stalled/missing", "fix/delete/delegate Y") carries `Premise: <what I assume> · seen: <read-only command> → <result>`. Without that line the play can only be "verify X", never "do X". A play nulled by a false premise still scores 0.5★, and the close-out reflection now counts them separately (`False premises: N`). The long R7 text was removed — the prompt got two lines shorter.
  Triggered by a 14-day audit (09-09 → 09-22), cross-checked by an independent model that saw the same data but not the agent's report: an unverified premise was the #1 cause of the weakest play (11 of 25), with R7 already written for weeks; effectiveness stayed flat at ~80% across 6 mutations in those 14 days, and the two lowest days (50%, 58%) were the same error twice — blaming a stall without reading the blocking ticket. Both reports agreed on the one thing *not* to do: add another rule that says "follow R7". A rule the agent has in context and still skips needs a different output shape, not more text. Measured at 09-30 with no other mutation in between, so the effect can be attributed.
- **v3.12 — approved 2026-09-20:** **self-knowledge that missed the first pass gets a guard, not just a memory (R9-c).** Content carrying backslashes, `\n`, backticks or `$` (a Windows-style path, a regex, a script with escaped code) has to be written with the editor's direct write, or from a temp file the script reads — never inline through an intermediate shell (`printf`, an unquoted heredoc, `python -c`, `node -e` with the text baked in, `sed` with the path inside the pattern). Each shell layer consumes one layer of escaping and executes any backtick along the way; the destination comes out mutilated and the command's own green says nothing about it. The finding had already been written down as knowledge (R9, v2.5) and still fired five times in 48 hours: two inline scripts mangled mid-write, a scheduled-task wrapper left without its path separators that then failed silently, a path committed with its separators eaten, a regex edit that silently stopped matching. Extends the scope of R9 (precedent v2.9/v3.0), no new rule — but this time the memory alone didn't hold, so it also ships its own pre-execution guard: a check that inspects the exact shell command before it runs and blocks the pattern. Proof of red: sabotaged against 14 real commands pulled from the session that produced it (5 that must block, 9 legitimate ones that must not) — caught a false positive in its own first draft (a quoted heredoc piped to an interpreter doesn't get expanded by the shell and was being blocked anyway) and a blind spot (a multi-line write hid its own backslashes past the point the check was reading up to). Fixed both, then watched it block, live, the exact kind of command it exists to stop.
- **v3.11 — approved 2026-09-19:** **adversarial review before the receipt (R11-b).** When the delivery is (i) a workflow the agent *rewrote* (topology or more than one node — not a one-field fix) or (ii) code touching a trust boundary (`api/`, `webhooks/`, the server, a third-party API), the unnamed dimension of R11 is supplied by **another head, not the author's own self-test**: the redacted diff/JSON, the list of changes and what to look for go to an adversarial reviewer (a different model) before the closing receipt or before the delivery leaves review. The receipt cites the reviewer's most severe finding and what was done about it. Outside those two cases the reviewer's quota is not spent. Caught something real twice before being written: a rewritten workflow (9→6 nodes, measured in production, receipt issued) where the reviewer found 3 bugs in one pass on the pasted JSON — one of them weeks old (U+FFFD replacement characters inside the keys of a column mapping → 3 columns written empty) that the author had looked at and **dismissed** by analogy with a harmless mojibake seen earlier; and two deliveries from another agent in the roster, both with a self-test in green, each with a security hole (auth bypass by header presence; query-language injection plus unbounded geometry) that only a full read caught. Extends the scope of R11 (precedent v2.9/v3.0/v3.10), no new rule; deliberately limited to two cases so the review budget isn't burned on trivial fixes.
- **v3.10 — approved 2026-09-19:** **the red runs where the guard lives (R6-b ii).** A control tested outside the script that hosts it — without its shell flags, its container, its runner — is not tested; and a "skip / could not test" branch of a guard is a red, not a pass. Two cases the same day on the same deploy canary: a probe run from inside a container with `|| echo skip` that let everything through (the container had `wget`, not `curl`), and a `grep` on a non-existent env file that under `set -euo pipefail` killed the deploy silently on that line — after three prior "greens" obtained by running the block loose, without `set -e`. Both reds existed; neither had ever executed in the place the guard was going to live. Caught something real twice before being written. Extends the scope of R6-b, no new rule.
- **v3.9 — approved 2026-09-13:** **the executor gets wired at close (R8).** Routing already required every play to name its cheapest capable executor. It caught something real the day it was extended: a session reviewed the open backlog looking for work to feed a delegate agent's autonomous loop — one that only consumes tickets labeled for it — and found zero such tickets, despite several plays tagged "executor: that agent" in previous closes. A play in prose never reaches a loop that reads tickets. Rule: a play delegated to a ticket-driven agent becomes its ticket in the same close; a play delegated to a cheap brief-driven model carries the alias and the brief (or its path) alongside. A delegated play without that wiring counts as a play without an executor. The cheapest executor in the roster (a cloud open-weights model used as "the bricklayer") is now listed explicitly in R8, fed with closed briefs by every agent, the reasoning one first.
- **v3.8 — approved 2026-09-12:** **off-keyboard actions are not plays (R12-d).** Paying, signing, calling, pasting a token, clicking through an external dashboard: the engine doesn't move them by proposing them. They go to a `⛔ Blockers — your court` block at close, each with an owner, a clickable recipe (the human said: "many of them I don't know how to do — tokens, lots of clicks"; without steps they don't happen) and what they unblock. The block is cumulative across rounds (the human batches them: "I gather them all and do them in one go"), so every close relists the live ones. And the agents never stall waiting: if the round depends on one, the play is what the engine can leave ready meanwhile. Why: the rule had existed since metric v1.6 — in the scoring step of the helper skill, not in the master prompt that builds the menu. Two consecutive rounds proposed them anyway (4 of 5 ignored plays were of this kind) and Effectiveness read 0.33/0.56 measuring something that wasn't aim. Same drift as v3.7, one rung down: the rule lived where the score is corrected, not where the decision is made.
- **v3.7 — approved 2026-09-12:** **menu hygiene — a rule the executor never reads governs nothing (R12).** A play takes one of 3 slots; if it doesn't earn it, it isn't proposed. Three measured forms of filler: (a) a detour while a priority thread has the clock running — close with that thread's next step instead of a menu; (b) structural filler — a play named only to discard it in the same turn, two options where one is a subset of the other, an adversarial review offered before the underlying decision is on the table, the engine's own chores (metrics, reflection) competing for a slot; (c) the play that converts is the one that closes what the round left hanging, not the one that opens a feature.
  Triggered while refactoring the helper skills by real usage: the 8 "consolidated rules" about the play menu lived in the memory file of a helper skill invoked a handful of times a month, which the opening hook never loads — only one had reached the master prompt, as a special case of the pre-proposal verifier. Three were genuinely missing and were promoted as a single rule so the effect can be attributed (if the "weakest play" stops being filler over the next 5 openings, it worked); the other five were already covered elsewhere and were not duplicated.
- **v3.6 — approved 2026-09-03:** **adversarial test — a green that only exercises what the play named proves nothing (R11).** If a play delivers code, a script or a monitor, its verification must **vary at least one dimension the play did not name**. A play that declares itself verified by testing only what it asked to fix is not verified: it measures that the fix does what it says, not that the system came out healthy. **The bug lives in the dimension that stayed constant.**
  Triggered by 2 of 2 deliveries on the same day from another agent in the roster, both arriving with a self-test in green and the bug still alive: a dedupe tested only with the collection in input order (breaks with the order reversed); a validator tested only with the field *absent* (passes with the field present but invalid, and fails open); a zone filter tested with a single zone (collides across scopes). In every case the delivery contract was **being met** — "every delivery ships a negative control" — the hole was not the absence of the control, it was its design.
  **The clause that makes it bite:** the varied dimension has to be an **orthogonal axis** to the change (order within the collection, shape of the malformed input, isolation scope, null or boundary state). Without it the rule is satisfied to the letter by a cosmetic dimension — *"I held the file encoding constant"* — and catches nothing. That clause came from the agent that the rule was written about, while trying to refute it: it found the case where the rule is obeyed and the bug still ships. The rule passed its own test.
  **Deliberately not a checklist:** an earlier draft enumerated those three cases as mandatory. Rejected — they are the three bugs of *that* day; as an obligation they become ceremony and leave out precisely the next dimension. They stay as examples.
  Why it belongs in the engine and not just in a style guide: without it a play can score 1.0 on the effectiveness metric using **its own green as the evidence** — the engine certifying itself. It is the same hole R6-b (PROOF OF RED) closed for monitors, one level up: R6-b demands you have *seen* a control go red; R11 demands the control *be able* to go red over something nobody asked it to watch.
